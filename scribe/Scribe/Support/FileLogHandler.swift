...
/// `~/Library/Logs/Scribe/`. This is the on-disk sink an AI debugging agent can read
...
nonisolated final class FileLogWriter: @unchecked Sendable {
    ...
    private static func makeLogFileURL() -> URL? {
        ...
        let bundleName = (Bundle.main.object(forInfoDictionaryKey: "CFBundleName") as? String) ?? "Scribe"
...
struct FileLogHandler: LogHandler {
    ...
    /// `com.bryanbernardo.scribe.runtime` → `runtime`. Matches `OSLogHandler`'s category convention so the

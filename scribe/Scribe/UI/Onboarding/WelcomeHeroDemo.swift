...
/// File overview:
/// The welcome step's hero: a self-playing demo of Jot's core loop (type, ghost text appears,
...
struct WelcomeHeroDemo: View {
    ...
    private var mockTextField: some View {
        ...
        // marks the focused text field. Sells "Scribe is live in this field right now."
        .overlay(
            RoundedRectangle(cornerRadius: 9, style: .continuous)
                .strokeBorder(ScribeBrand.accent.opacity(0.65), lineWidth: 1)
        )
        .overlay(
            RoundedRectangle(cornerRadius: 9, style: .continuous)
                .inset(by: -2.5)
                .strokeBorder(ScribeBrand.accent.opacity(0.22), lineWidth: 3)
        )
    }

...
/// File overview:
/// Adapts Apple's on-device Foundation Models framework to Jot's existing
...
final class FoundationModelSuggestionEngine {
    ...
    /// Maps Jot's existing generation knobs onto the subset of Foundation Models options the
    ...
    /// Converts framework-specific failures into Jot's existing error vocabulary so the rest of

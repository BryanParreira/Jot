...
struct FocusSnapshotResolver {
    ...
    /// the focus provider after Jot's own synthetic insert: the cached run texts predate the
    ...
    /// Extracts the AX properties Jot needs from one candidate element near the current focus.
    ...
    /// Detects secure inputs so Jot can intentionally refuse to operate in sensitive fields.

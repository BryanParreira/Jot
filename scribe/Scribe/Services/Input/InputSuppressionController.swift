...
/// File overview:
/// Tracks Jot's own synthetic key events so inserted suggestions do not recursively trigger
...
final class InputSuppressionController {
    ...
    /// Stamped onto Jot's synthetic insertion events so any tap can recognize them by identity,
    ...
    /// True when the event carries Jot's synthetic-insertion marker.

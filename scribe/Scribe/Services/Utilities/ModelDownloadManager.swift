...
final class ModelDownloadManager: ObservableObject {
    ...
    /// Jot's own directory first (authoritative + download target), then any additive sources
    ...
    /// The path shown in Settings and opened by "Open Folder". This is always Jot's own writable
    ...
    /// Returns `true` only when the model lives in Jot's user-writable model directory.

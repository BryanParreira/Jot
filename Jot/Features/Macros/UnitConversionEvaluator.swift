...
/// Physical-unit conversion macros, fully offline via Foundation `Measurement`:
/// `/10km->mi`, `/100f->c`, `/5ft->m`, `/2lb->kg`, `/3cup->ml`.
/// From and to tokens must belong to the same quantity; cross-quantity requests
/// return nil. Output is locale-formatted.
struct UnitConversionEvaluator: MacroEvaluating {
    ...
    private static func convert(_ value: Double, from: Quantity, to: Quantity) -> Double? {
        switch (from, to) {
        case let (.length(s), .length(t)):
            return Measurement(value: value, unit: s).converted(to: t).value
        case let (.mass(s), .mass(t)):
            return Measurement(value: value, unit: s).converted(to: t).value
        case let (.temperature(s), .temperature(t)):
            return Measurement(value: value, unit: s).converted(to: t).value
        case let (.volume(s), .volume(t)):
            return Measurement(value: value, unit: s).converted(to: t).value
        default:
            return nil
        }
